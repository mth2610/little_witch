...
uniform float time;
uniform float rotation;   // Góc xoay phi tiêu (radian)
...
void main() {
    ...
    float bladeShape = 0.35 + (blade * 0.5) * 0.65;
    ...
    float trail = 0.0;
    // GLES 2.0 loop must have constant/literal bounds (i < 4 is constant)
    for (int i = 0; i < 4; i++) {
    ...
    float rays = 0.0;
    for (int i = 0; i < 6; i++) {
        float rayAngle = float(i) * 1.047197 + rotation * 1.2;
