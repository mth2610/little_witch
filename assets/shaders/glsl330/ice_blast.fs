// ice_blast.fs — Tinh thể băng Voronoi lấp lánh & sương tuyết cực đỉnh (Hệ Thủy)
#version 330
...
void main() {
    ...
    float borderFade = smoothstep(shape, shape - 0.04, dist);
    ...
    float crackLine = smoothstep(0.05, 0.0, crack);
    ...
    // Đọc màu nền bị méo
    vec3 bg = texture(screenTexture, distortedUV).rgb;
