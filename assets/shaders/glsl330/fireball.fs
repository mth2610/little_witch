// fireball.fs — Quả cầu lửa volumetric 3D nâng cấp từ thuật toán Oz (Hệ Hỏa)
#version 330
...
void main() {
    ...
    for (int i = 0; i < 35; i++) {
        ...
        // q.y (chiều cao tail) = -p.x (phía sau bóng)
        // q.x = p.y
        // q.z = p.z
    ...
    // Nén màu sắc qua hàm tanh để tránh bị cháy sáng trắng quá đà (High Dynamic Range mapping)
    vec3 rgb = tanh(fireColor * d_acc / z / 100.0);
    ...
    finalColor = vec4(rgb * (0.95 + 0.45 * intensity), alpha);
}
