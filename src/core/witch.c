...
void DrawWitch(const Witch *witch) {
    ...
    if (textureReady) {
        if (hasShieldTextureDraw) {
            float radiusVal = (100.0f + 12.0f * witch->effectiveThuyStars) + sinf((float)GetTime() * 7.0f) * 5.0f;
            DrawShieldEffect(witch->position, radiusVal);
        }
        ...
        if (witch->animState == WITCH_STATE_ATTACK_HAND) {
            ...
            float radiusVal = 100.0f + sinf((float)GetTime() * 7.0f) * 5.0f;
            DrawShieldEffect(pos, radiusVal);
        }
    }
}
