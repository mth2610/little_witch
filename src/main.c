...
static void UpdateGameplay(GameState *gs, float deltaTime) {
  ...
  if (IsKeyPressed(KEY_SPACE) || IsKeyDown(KEY_SPACE) ||
    ...
    if (gs->witch.keyboardAttackCooldown <= 0.0f) {
      ...
      // Lý do: Đảm bảo đòn đánh thường yếu hoạt động chính xác khi có 0 sao và
      // đòn đánh chuẩn hoạt động khi có >= 1 sao
      CastNormalAttack(gs);
...
int main(void) {
  ...
  // Trên máy tính chạy chế độ cửa sổ to 1280x720, trên Android tự động giãn
  // toàn màn hình
#ifdef __ANDROID__
...
#undef SHADER_PATH
  ...
  while (!WindowShouldClose()) {
    ...
    if (gs.currentScreen == SCREEN_GAMEPLAY ||
        gs.currentScreen == SCREEN_SKILL_SELECT ||
        gs.currentScreen == SCREEN_GAMEOVER) {
      // deltaTime được truyền là 0 khi đứng yên ở card chọn kỹ năng để dừng
      // cuộn nền
      ...
      // Chụp màn hình hiện tại (background, stars, enemies) để truyền làm
      // screenTexture cho Distortion
      EndTextureMode();
      ...
      // Chọn màu tối của Biome hiện tại (Ambient Darkness - tối hơn để tăng độ
      // sâu và tương phản phong cách Ori)
      ...
      // A. Ánh sáng phát ra từ Phù thủy (Màu xanh cyan/trắng dịu phong cách
      // Ori)
      ...
      // 112x112px (tránh mờ nhạt/giống ma)
      ...
      // Nếu khiên ma pháp đang hoạt động, tỏa ra vầng sáng bảo hộ khổng lồ màu
      // xanh trời trên lightmap
      if (gs.witch.manaShieldHealth > 0.0f) {
      ...
      // B. Ánh sáng phát ra từ các ngôi sao bảo vệ/sao tự do (Ori-style glowing
      // stars)
      for (int i = 0; i < MAX_STARS; i++) {
        if (gs.starPool.stars[i].active) {
          ...
          // Lõi sáng trắng cực mạnh ở tâm với bán kính lớn hơn để vật thể giữ
          // màu gốc tốt hơn
        ...
        if (gs.projectilePool.projectiles[i].active) {
          ...
          if (p->isEnemy) {
            ...
            if (p->type == PROJ_FIREBALL) {
              // Vầng sáng cam/đỏ rộng 80px, lõi vàng trắng, nhấp nháy nhanh
              // giống ngọn lửa
              ...
              // Vầng sáng cyan nhạt 60px, tĩnh lặng, hơi sương trắng mỏng xung
              // quanh
              float size = 60.0f;
              DrawCircleGradient(p->position.x, p->position.y, size * 1.5f,
                                 ColorAlpha(WHITE, 0.25f),
                                 ColorAlpha(WHITE, 0.0f)); // Sương trắng mỏng
              DrawCircleGradient(p->position.x, p->position.y, size,
              ...
              // Vầng sáng dọc theo chiều cao lốc xoáy, cam-vàng, flash sét
              // trắng ngẫu nhiên
              ...
              for (int step = 0; step < 5; step++) {
                ...
                //     offsety, size * 0.3f, ColorAlpha(WHITE, 0.8f),
                //     ColorAlpha(WHITE, 0.0f));
                // }
              }
            } else {
      ...
      // E. Ánh sáng phát ra từ quái vật và Boss (giúp chúng rực rỡ phong cách
      // Ori)
      for (int i = 0; i < MAX_ENEMIES; i++) {
        if (gs.enemyPool.enemies[i].active) {
          ...
          // Vẽ lõi sáng trắng cực mạnh để quái giữ nguyên màu sắc gốc và nổi
          // bật
      ...
      // G. Ánh sáng phát ra từ môi trường (Đom đóm, Tinh thể, Tinh vân phong
      // cách Ori)
      if (gs.currentBiome == BIOME_FOREST) {
    ...
    // Áp dụng rung giật màn hình (Screen Shake) lên tọa độ canvas đích nếu có
    // và đang ở Gameplay
  ...
  if (gs.fireballShaderLoaded)
    UnloadShader(gs.fireballShader);
  if (gs.iceBlastShaderLoaded)
    UnloadShader(gs.iceBlastShader);
  if (gs.poisonCloudShaderLoaded)
    UnloadShader(gs.poisonCloudShader);
  if (gs.shurikenShaderLoaded)
    UnloadShader(gs.shurikenShader);
  if (gs.tornadoShaderLoaded)
    UnloadShader(gs.tornadoShader);
