...
void UpdateFluidSkill(struct GameState *gs, float dt) {
  ...
  // 2. CẬP NHẬT CÁC HẠT NƯỚC VÀ XỬ LÝ VA CHẠM
  for (int i = 0; i < MAX_FLUID_PARTICLES; i++) {
    ...
    if (waterPool[i].type == 2) { // Splash
      ...
      for (int e = 0; e < MAX_ENEMIES; e++) {
        if (gs->enemyPool.enemies[e].active) {
          ...
          float dist = Vector2Distance(waterPool[i].position, enemy->position);
          if (dist < (waterPool[i].radius * 0.5f + enemy->collisionRadius)) {
            int lvl = gs->witch.skillLevels[SKILL_ICE_BLAST];
            int damage = (10 + 4 * gs->witch.effectiveThuyStars) *
                         (1.0f + 0.15f * (lvl - 1));

            bool killed = DamageEnemy(&gs->enemyPool, e, damage);
            if (killed) {
    ...
    for (int e = 0; e < MAX_ENEMIES; e++) {
      if (gs->enemyPool.enemies[e].active) {
        ...
        float dist = Vector2Distance(waterPool[i].position, enemy->position);
        if (dist < (waterPool[i].radius * 0.6f + enemy->collisionRadius)) {
          int lvl = gs->witch.skillLevels[SKILL_ICE_BLAST];
          int damage = (15 + 6 * gs->witch.effectiveThuyStars) *
                       (1.0f + 0.18f * (lvl - 1));
          ...
          // Nổ bọt nước khi trúng địch
          waterPool[i].active = false;
          int splashCount = GetRandomValue(4, 7);
          for (int s = 0; s < splashCount; s++) {
            Vector2 splashVel = {(float)GetRandomValue(-300, 300),
                                 (float)GetRandomValue(-400, 100)};
            float splashRad = (float)GetRandomValue(6, 18);
            float splashLife = (float)GetRandomValue(4, 8) / 10.0f;
            SpawnFluidParticle(2, waterPool[i].position, splashVel, 0, 0,
                               splashLife, splashRad, NULL);
          }
          break;
    ...
    if (GetRandomValue(1, 100) <= 8) {
      Vector2 forwardDir = Vector2Normalize(
          Vector2Subtract(waterPool[i].targetPos, waterPool[i].position));
      Vector2 dropVel = {
      ...
      SpawnFluidParticle(2, waterPool[i].position, dropVel, 0, 0, mistLife,
                         mistRad, NULL);
    }
  }
}
