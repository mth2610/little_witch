...
#define WOOD_PROGRESS_MAX 2.4f  // Allow roots to persist on screen after hitting target
...
static Vector2 GetWoodPointAt(const WoodEmitter *em, float t, int branchIndex) {
    if (t <= 1.0f) {
        ...
        float contactAngle = atan2f(em->contactPos.y - em->targetPos.y, em->contactPos.x - em->targetPos.x);
...
static void DrawProceduralLeafUV(Vector2 position, float radius, float angle, float fade) {
    ...
    rlBegin(RL_TRIANGLES);
      ...
      rlColor4ub(cTail.r, cTail.g, cTail.b, cTail.a);   rlVertex2f(tail.x, tail.y);
      rlColor4ub(cLeft.r, cLeft.g, cLeft.b, cLeft.a);   rlVertex2f(left.x, left.y);
      rlColor4ub(cHead.r, cHead.g, cHead.b, cHead.a);   rlVertex2f(head.x, head.y);

      // Triangle 2: tail, head, right
      rlColor4ub(cTail.r, cTail.g, cTail.b, cTail.a);   rlVertex2f(tail.x, tail.y);
      rlColor4ub(cHead.r, cHead.g, cHead.b, cHead.a);   rlVertex2f(head.x, head.y);
      rlColor4ub(cRight.r, cRight.g, cRight.b, cRight.a); rlVertex2f(right.x, right.y);
    rlEnd();
}
...
static void DrawProceduralFlowerUV(Vector2 position, float radius, float angle, float fade) {
    ...
    for (int i = 0; i < 4; i++) {
        rotated[i] = (Vector2){
            position.x + corners[i].x * c - corners[i].y * s,
    ...
    rlBegin(RL_TRIANGLES);
      rlColor4ub(c0.r, c0.g, c0.b, c0.a); rlVertex2f(rotated[0].x, rotated[0].y);
      rlColor4ub(c1.r, c1.g, c1.b, c1.a); rlVertex2f(rotated[1].x, rotated[1].y);
      rlColor4ub(c3.r, c3.g, c3.b, c3.a); rlVertex2f(rotated[3].x, rotated[3].y);

      rlColor4ub(c0.r, c0.g, c0.b, c0.a); rlVertex2f(rotated[0].x, rotated[0].y);
      rlColor4ub(c3.r, c3.g, c3.b, c3.a); rlVertex2f(rotated[3].x, rotated[3].y);
      rlColor4ub(c2.r, c2.g, c2.b, c2.a); rlVertex2f(rotated[2].x, rotated[2].y);
    rlEnd();
}
...
static void DrawProceduralShardUV(Vector2 position, float radius, float angle, float fade) {
    ...
    rlBegin(RL_TRIANGLES);
      rlColor4ub(cLeft.r, cLeft.g, cLeft.b, cLeft.a);   rlVertex2f(left.x, left.y);
      rlColor4ub(cRight.r, cRight.g, cRight.b, cRight.a); rlVertex2f(right.x, right.y);
      rlColor4ub(cTip.r, cTip.g, cTip.b, cTip.a);     rlVertex2f(tip.x, tip.y);
    rlEnd();
}
...
// Particle spawning
static void SpawnWoodParticle(int type, Vector2 pos, Vector2 vel, float baseRadius, float maxLife, float angle, float spinSpeed) {
...
void InitWoodSkill(int screenWidth, int screenHeight) {
    ...
    for (int i = 0; i < MAX_WOOD_PARTICLES; i++) woodPool[i].active = false;
...
void CastWoodSkill(Vector2 startPos, Vector2 target, int branchCount) {
    ...
    em->p1 = Vector2Add(Vector2Add(startPos, Vector2Scale(dir, dist * 0.35f)), Vector2Scale(perp, curveSign * dist * 0.25f));
    em->p2 = Vector2Add(Vector2Add(startPos, Vector2Scale(dir, dist * 0.70f)), Vector2Scale(perp, -curveSign * dist * 0.15f));

    // Calculate contactPos on target's outer boundary (radius 44.0 for initial python-coil entry)
    float wrapRadius = 44.0f;
    Vector2 dirToTarget = Vector2Normalize(Vector2Subtract(target, em->p2));
    em->contactPos = Vector2Subtract(target, Vector2Scale(dirToTarget, wrapRadius));

    // Caster nature burst: 2 layered expanding shockwaves (Green/leaf energy)
    SpawnWoodParticle(PARTICLE_ENERGY_FLASH, startPos, (Vector2){0,0}, 55.0f, 0.5f, 0, 0);
    SpawnWoodParticle(PARTICLE_ENERGY_FLASH, startPos, (Vector2){0,0}, 90.0f, 0.35f, 0, 0);

    // Swirl of 15 leaves spinning out from caster
    for (int s = 0; s < 15; s++) {
        float angle = ((float)s / 15.0f) * PI * 2.0f + Random01() * 0.3f;
        float speed = Math_Mix(140.0f, 280.0f, Random01());
        Vector2 vel = {cosf(angle) * speed, sinf(angle) * speed};
        SpawnWoodParticle(
            PARTICLE_LEAF, 
            startPos, 
            vel, 
            Math_Mix(8.0f, 15.0f, Random01()), 
            Math_Mix(0.8f, 1.4f, Random01()), 
            angle, 
...
void UpdateWoodSkill(float dt) {
    ...
    for (int e = 0; e < MAX_EMITTERS; e++) {
        ...
        emitters[e].progress += dt * WOOD_TRAVEL_SPEED * emitters[e].speedMultiplier;
        ...
        if (emitters[e].progress <= 1.8f) {
            ...
            if (emitters[e].sproutAccumulator >= 0.04f) {
                ...
                for (int b = 0; b < emitters[e].branchCount; b++) {
                    if (emitters[e].progress <= 1.0f && b > 0) continue;

                    Vector2 headPos = GetWoodPointAt(&emitters[e], emitters[e].progress, b);
                    Vector2 tangent;
                    float prevProg = fmaxf(0.0f, emitters[e].progress - 0.01f);
                    if (emitters[e].progress > prevProg) {
                        tangent = Vector2Normalize(Vector2Subtract(headPos, GetWoodPointAt(&emitters[e], prevProg, b)));
                    } else {
                    ...
                    Vector2 leafVel = Vector2Scale(Vector2Add(Vector2Scale(perp, side * 80.0f), Vector2Scale(tangent, -40.0f)), Random01());
                    SpawnWoodParticle(
                        PARTICLE_LEAF, 
                        headPos, 
                        leafVel, 
                        Math_Mix(4.0f, 9.0f, Random01()), // slightly smaller leaves for performance when branching
                        Math_Mix(1.0f, 1.8f, Random01()), 
                        Random01() * PI * 2.0f, 
                        Math_Mix(-4.0f, 4.0f, Random01())
                    );

                    // Spawn some glowing pollen dust
                    Vector2 pollenVel = { (Random01() - 0.5f) * 60.0f, -Random01() * 40.0f };
                    SpawnWoodParticle(
                        PARTICLE_POLLEN, 
                        headPos, 
                        pollenVel, 
                        Math_Mix(1.5f, 3.5f, Random01()), 
        ...
        if (prevProgress <= 1.0f && emitters[e].progress > 1.0f) {
            ...
            // Spawn dynamic blooming flowers (fewer and smaller) at the contact position on the enemy boundary
            SpawnWoodParticle(PARTICLE_FLOWER, contactPos, (Vector2){0, 0}, 24.0f, 1.4f, Random01()*PI, 0.4f);

            // Explosion leaf burst
            int leafBurst = GetRandomValue(10, 18);
            for (int s = 0; s < leafBurst; s++) {
                float angle = Random01() * PI * 2.0f;
                float speed = Math_Mix(80.0f, 300.0f, Random01());
                Vector2 vel = {cosf(angle) * speed, sinf(angle) * speed - 50.0f};
                SpawnWoodParticle(
                    PARTICLE_LEAF, 
                    hitPos, 
                    vel, 
                    Math_Mix(6.0f, 12.0f, Random01()), 
                    Math_Mix(1.2f, 2.2f, Random01()), 
                    Random01()*PI*2.0f, 
            ...
            int pollenBurst = GetRandomValue(12, 22);
            for (int s = 0; s < pollenBurst; s++) {
                float angle = Random01() * PI * 2.0f;
                float speed = Math_Mix(40.0f, 150.0f, Random01());
                Vector2 vel = {cosf(angle) * speed, sinf(angle) * speed - 30.0f};
                SpawnWoodParticle(
                    PARTICLE_POLLEN, 
                    hitPos, 
                    vel, 
                    Math_Mix(2.0f, 5.0f, Random01()), 
                    Math_Mix(1.0f, 2.0f, Random01()), 
                    0, 
                    0
                );
            ...
            int shardBurst = GetRandomValue(8, 14);
            for (int s = 0; s < shardBurst; s++) {
                float angle = Random01() * PI * 2.0f;
                float speed = Math_Mix(150.0f, 450.0f, Random01());
                Vector2 vel = {cosf(angle) * speed, sinf(angle) * speed};
                SpawnWoodParticle(
                    PARTICLE_WOOD_SHARD, 
                    hitPos, 
                    vel, 
                    Math_Mix(4.0f, 10.0f, Random01()), 
                    Math_Mix(0.6f, 1.0f, Random01()), 
                    Random01()*PI*2.0f, 
        ...
        if (prevProgress <= 1.8f && emitters[e].progress > 1.8f) {
            for (int b = 0; b < emitters[e].branchCount; b++) {
                ...
                // Spawn 1 small flower at the tip of each branch (radius 22.0f for visibility)
                SpawnWoodParticle(PARTICLE_FLOWER, hitPos, (Vector2){0, 0}, 22.0f, 1.3f, Random01()*PI, -0.5f);

                // Explosion pollen bloom
                int pollenBurst = GetRandomValue(5, 9);
                for (int s = 0; s < pollenBurst; s++) {
                    float angle = Random01() * PI * 2.0f;
                    float speed = Math_Mix(30.0f, 80.0f, Random01());
                    Vector2 vel = {cosf(angle) * speed, sinf(angle) * speed};
                    SpawnWoodParticle(
                        PARTICLE_POLLEN, 
                        hitPos, 
        ...
        if (emitters[e].progress > 1.8f) {
            ...
            if (emitters[e].sproutAccumulator >= 0.05f) {
                ...
                Vector2 crumblePos = GetWoodPointAt(&emitters[e], randomT, randomBranch);

                Vector2 decayVel = { (Random01() - 0.5f) * 40.0f, Math_Mix(60.0f, 160.0f, Random01()) };
...
void DrawWoodSkill(void) {
    ...
    for (int e = 0; e < MAX_EMITTERS; e++) {
        ...
        float maxThick = emitters[e].isSubBranch ? 11.0f : 16.0f;
        ...
        // 1. Draw Main Stem
        {
            ...
            if (t_start < t_end) {
                ...
                for (int i = 0; i < numPoints - 1; i++) {
                    float thick_i = GetWoodThickness(tValues[i], maxThick, emitters[e].branchCount);
                    float thick_ip1 = GetWoodThickness(tValues[i + 1], maxThick, emitters[e].branchCount);

                    Vector2 tangent_i;
                    if (i == 0) {
                        tangent_i = Vector2Normalize(Vector2Subtract(purePos[1], purePos[0]));
                    } else {
                        tangent_i = Vector2Normalize(Vector2Subtract(purePos[i + 1], purePos[i - 1]));
                    }
                    Vector2 perp_i = (Vector2){ -tangent_i.y, tangent_i.x };

                    Vector2 tangent_ip1;
                    if (i + 1 == numPoints - 1) {
                        tangent_ip1 = Vector2Normalize(Vector2Subtract(purePos[numPoints - 1], purePos[numPoints - 2]));
                    } else {
                        tangent_ip1 = Vector2Normalize(Vector2Subtract(purePos[i + 2], purePos[i]));
                    }
                    Vector2 perp_ip1 = (Vector2){ -tangent_ip1.y, tangent_ip1.x };

                    Vector2 V0 = Vector2Subtract(purePos[i], Vector2Scale(perp_i, thick_i * 0.5f));
                    Vector2 V1 = Vector2Add(purePos[i], Vector2Scale(perp_i, thick_i * 0.5f));
                    Vector2 V2 = Vector2Subtract(purePos[i + 1], Vector2Scale(perp_ip1, thick_ip1 * 0.5f));
                    Vector2 V3 = Vector2Add(purePos[i + 1], Vector2Scale(perp_ip1, thick_ip1 * 0.5f));
                    ...
                    rlBegin(RL_TRIANGLES);
                      rlColor4ub(C0.r, C0.g, C0.b, C0.a); rlVertex2f(V0.x, V0.y);
                    ...
                    if (i % 14 == 0 && i > 6 && i < numPoints - 6) {
                        ...
                        float thornLen = thick_i * 1.8f;
                        Vector2 tip = Vector2Add(purePos[i], Vector2Scale(perp_i, side * thornLen));
                        Vector2 base1 = Vector2Add(purePos[i], Vector2Scale(tangent_i, thick_i * 0.7f));
                        Vector2 base2 = Vector2Subtract(purePos[i], Vector2Scale(tangent_i, thick_i * 0.7f));

                        Color cThorn = (Color){ (unsigned char)(tValues[i] * 255.0f), (unsigned char)(side == 1.0f ? 255 : 0), 0, aVal };
                        rlBegin(RL_TRIANGLES);
                          rlColor4ub(cThorn.r, cThorn.g, cThorn.b, cThorn.a); rlVertex2f(base1.x, base1.y);
                          rlColor4ub(cThorn.r, cThorn.g, cThorn.b, cThorn.a); rlVertex2f(base2.x, base2.y);
                          rlColor4ub(cThorn.r, cThorn.g, cThorn.b, cThorn.a); rlVertex2f(tip.x, tip.y);
                        rlEnd();
                    }
                }
            }
            ...
            if (t_start < t_end) {
                ...
                for (int b = 0; b < emitters[e].branchCount; b++) {
                    ...
                    for (int i = 0; i < numPoints - 1; i++) {
                        ...
                        float thick_i = GetWoodThickness(tValues[i], maxBranchThick, emitters[e].branchCount + b);
                        float thick_ip1 = GetWoodThickness(tValues[i + 1], maxBranchThick, emitters[e].branchCount + b);
    ...
    for (int e = 0; e < MAX_EMITTERS; e++) {
        ...
        float maxLeafRad = emitters[e].isSubBranch ? 8.0f : 11.0f;
        ...
        for (float t = 0.05f; t < stemProgress - 0.02f; t += 0.08f) {
            ...
            Vector2 tangent = Vector2Normalize(Vector2Subtract(GetWoodPointAt(&emitters[e], t + 0.01f, 0), GetWoodPointAt(&emitters[e], fmaxf(0.0f, t - 0.01f), 0)));
        ...
        if (emitters[e].progress > 1.0f) {
            ...
            for (int b = 0; b < emitters[e].branchCount; b++) {
                for (float t = 1.05f; t < branchProgress - 0.02f; t += 0.10f) {
                    ...
                    Vector2 tangent = Vector2Normalize(Vector2Subtract(GetWoodPointAt(&emitters[e], t + 0.01f, b), GetWoodPointAt(&emitters[e], t - 0.01f, b)));
                    float baseAngle = atan2f(tangent.y, tangent.x);
    ...
    for (int i = 0; i < MAX_WOOD_PARTICLES; i++) {
        ...
        if (woodPool[i].type == PARTICLE_LEAF) {
            DrawProceduralLeafUV(woodPool[i].position, woodPool[i].radius * (0.3f + 0.7f * lifeRatio), woodPool[i].angle, lifeRatio);
        } 
        else if (woodPool[i].type == PARTICLE_POLLEN) {
            DrawProceduralFlowerUV(woodPool[i].position, woodPool[i].radius * (0.5f + 0.5f * lifeRatio), 0.0f, lifeRatio);
        }
        else if (woodPool[i].type == PARTICLE_WOOD_SHARD) {
            DrawProceduralShardUV(woodPool[i].position, woodPool[i].radius, woodPool[i].angle, lifeRatio);
        }
        else if (woodPool[i].type == PARTICLE_FLOWER) {
            float sizeMult = smoothstep(1.0f, 0.7f, lifeRatio);
            DrawProceduralFlowerUV(woodPool[i].position, woodPool[i].radius * sizeMult, woodPool[i].angle, lifeRatio);
        }
        else if (woodPool[i].type == PARTICLE_ENERGY_FLASH) {
            DrawProceduralEnergyFlashUV(woodPool[i].position, woodPool[i].radius, lifeRatio);
        }
    }
    EndBlendMode();
...
bool IsWoodSkillCoiling(void) {
    for (int e = 0; e < MAX_EMITTERS; e++) {
        if (emitters[e].active && emitters[e].progress >= 1.0f && emitters[e].progress <= 1.8f) {
            return true;
        }
    }
    return false;
