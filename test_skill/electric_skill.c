...
void CastElectricSkill(Vector2 startPos, Vector2 target) {
    ...
    // Spark blast at casting origin
    for (int s = 0; s < 15; s++) {
        float angle = Random01() * 2.0f * 3.14159265f;
        float speed = 100.0f + Random01() * 250.0f;
        Vector2 vel = { cosf(angle) * speed, sinf(angle) * speed };
        SpawnParticle(PARTICLE_SPARK, startPos, vel, 3.0f + Random01() * 4.0f, 0.4f + Random01() * 0.4f);
    }
    
    // Initial plasma muzzle burst
    SpawnParticle(PARTICLE_SHOCKWAVE, startPos, (Vector2){0, 0}, 30.0f, 0.35f);
}
    ...
    for (int e = 0; e < MAX_EMITTERS; e++) {
        ...
        if (!emitters[e].impacted) {
            ...
            if (emitters[e].progress >= 1.0f) {
                ...
                SpawnParticle(PARTICLE_SHOCKWAVE, emitters[e].targetPos, (Vector2){0, 0}, 80.0f, 0.5f);
                SpawnParticle(PARTICLE_SHOCKWAVE, emitters[e].targetPos, (Vector2){0, 0}, 130.0f, 0.7f);

                // B. Explosion particles
                for (int s = 0; s < 35; s++) {
                    float angle = Random01() * 2.0f * 3.14159265f;
                    float speed = 150.0f + Random01() * 450.0f;
                    Vector2 vel = { cosf(angle) * speed, sinf(angle) * speed };
                    SpawnParticle(PARTICLE_BURST_SPARK, emitters[e].targetPos, vel, 4.0f + Random01() * 5.0f, 0.5f + Random01() * 0.5f);
                }

                // C. Discharges around the enemy
                for (int s = 0; s < 8; s++) {
                    Vector2 offset = { (float)GetRandomValue(-40, 40), (float)GetRandomValue(-40, 40) };
                    SpawnParticle(PARTICLE_ARC, Vector2Add(emitters[e].targetPos, offset), (Vector2){0, 0}, 2.0f, 0.25f);
                }
