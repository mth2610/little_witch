...
void CastMetalSkill(Vector2 startPos, Vector2 target, int count) {
    ...
    for (int j = 0; j < count; j++) {
        ...
        em->portalPositions[j] = Vector2Add(startPos, Vector2Scale(perp, offsetFactor * 40.0f));
        
        // Thiết lập kích thước khác nhau (Cổng trung tâm to nhất, hai bên nhỏ dần)
        float distFromCenter = fabsf(offsetFactor);
        if (distFromCenter < 0.1f) {
            em->portalSizes[j] = 1.15f; // Kiếm chính trung tâm
        } else if (distFromCenter < 1.1f) {
            em->portalSizes[j] = 0.85f; // Kiếm phụ hai bên
        } else {
            em->portalSizes[j] = 0.65f; // Kiếm nhỏ ngoài cùng
        }
    }
    ...
    for (int i = 0; i < 8; i++) {
        ...
        SpawnMetal(PARTICLE_SPARK, startPos, flashVel, (float)GetRandomValue(8, 16), (float)GetRandomValue(1, 2), 0.2f, zeroTarget, 0.0f, 0.0f, 1.0f);
    }
}
        ...
        for (int j = 0; j < emitters[e].count; j++) {
            if (!emitters[e].spawned[j]) {
                ...
                if (emitters[e].timer >= emitters[e].spawnDelay[j]) {
                    ...
                    float swordLen = (float)GetRandomValue(80, 110) * sizeFactor;
                    float swordThick = (float)GetRandomValue(15, 21) * sizeFactor;
                    ...
                    for (int s = 0; s < 6; s++) {
                        ...
                        SpawnMetal(PARTICLE_SPARK, spawnPos, sparkVel, (float)GetRandomValue(6, 16), (float)GetRandomValue(1, 2), 0.2f, zeroTarget, 0.0f, 0.0f, sizeFactor);
                    }
                }
            }
        }
